...
static _inline void arp_cache_lock_init(void) {
  ...
  if (pthread_mutexattr_init(&attr) != 0) perrx("pthread_mutexattr_init");
  if (pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_NORMAL) != 0) perrx("pthread_mutexattr_settype");
  if (pthread_mutex_init(&arp_cache_mutex, &attr) != 0) perrx("pthread_mutex_init");
#endif
}

/* Lock Function */
#ifdef DEBUG_ARPCACHE_LOCK
#define arp_cache_lock()                  \
  do {                                    \
    dbg("lock");                          \
    pthread_mutex_lock(&arp_cache_mutex); \
  } while (0)
#define arp_cache_unlock()                  \
  do {                                      \
    dbg("unlock");                          \
    pthread_mutex_unlock(&arp_cache_mutex); \
  } while (0)
#else
static _inline void arp_cache_lock(void) { pthread_mutex_lock(&arp_cache_mutex); }

static _inline void arp_cache_unlock(void) { pthread_mutex_unlock(&arp_cache_mutex); }
...
struct arpentry *arp_alloc(void) {
  ...
  for (i = 0; i < ARP_CACHE_SZ; i++) {
    if (arp_cache[next].ae_state == ARP_FREE) break;
...
int arp_insert(struct netdev *nd, unsigned short pro, unsigned int ipaddr, unsigned char *hwaddr) {
  struct arpentry *ae;
  ae = arp_alloc();
  if (!ae) return -1;
  ae->ae_dev = nd;
...
struct arpentry *arp_lookup(unsigned short pro, unsigned int ipaddr) {
  ...
  for (ae = arp_cache_head; ae < arp_cache_end; ae++) {
    if (ae->ae_state == ARP_FREE) continue;
...
struct arpentry *arp_lookup_resolv(unsigned short pro, unsigned int ipaddr) {
  ...
  if (ae && ae->ae_pro == ARP_RESOLVED) return ae;
  return NULL;
}
...
void arp_timer(int delta) {
  ...
  for (ae = arp_cache_head; ae < arp_cache_end; ae++) {
    ...
    ae->ae_ttl -= delta;
    if (ae->ae_ttl <= 0) {
      if ((ae->ae_state == ARP_WAITING && --ae->ae_retry < 0) || ae->ae_state == ARP_RESOLVED) {
        if (ae->ae_state == ARP_WAITING) arp_queue_drop(ae);
...
void arp_cache_init(void) {
  int i;
  for (i = 0; i < ARP_CACHE_SZ; i++) arp_cache[i].ae_state = ARP_FREE;
  dbg("ARP CACHE INIT");
  arp_cache_lock_init();
  dbg("ARP CACHE SEMAPHORE INIT");
}

static const char *__arpstate[] = {NULL, "Free", "Waiting", "Resolved"};

static _inline const char *arpstate(struct arpentry *ae) { return __arpstate[ae->ae_state]; }
...
void arp_cache_traverse(void) {
  ...
  for (ae = arp_cache_head; ae < arp_cache_end; ae++) {
    ...
    printf("%-9s%-12d" MACFMT " %s\n", arpstate(ae), ((ae->ae_ttl < 0) ? 0 : ae->ae_ttl), macfmt(ae->ae_hwaddr), ipnfmt(ae->ae_ipaddr));
  }
  arp_cache_unlock();
}
