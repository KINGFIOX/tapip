...
void arp_request(struct arpentry *ae) {
  ...
  arpdbg(IPFMT "(" MACFMT ")->" IPFMT "(request)", ipfmt(ahdr->arp_sip), macfmt(ahdr->arp_sha), ipfmt(ahdr->arp_tip));
...
void arp_recv(struct netdev *dev, struct pkbuf *pkb) {
  ...
  if (ae) {
    ...
    if (ae->ae_state == ARP_WAITING) arp_queue_send(ae);
...
void arp_in(struct netdev *dev, struct pkbuf *pkb) {
  ...
  if (ahdr->arp_hrd != ARP_HRD_ETHER || ahdr->arp_pro != ETH_P_IP || ahdr->arp_hrdlen != ETH_ALEN || ahdr->arp_prolen != IP_ALEN) {
