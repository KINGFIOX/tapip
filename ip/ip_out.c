...
void ip_send_dev(struct netdev *dev, struct pkbuf *pkb) {
  ...
  if (rt->rt_flags & RT_LOCALHOST) {
    ipdbg("To loopback");
    netdev_tx(dev, pkb, pkb->pk_len - ETH_HRD_SZ, ETH_P_IP, dev->net_hwaddr);
    return;
  }
  ...
  if (!ae) {
    ...
    netdev_tx(dev, pkb, pkb->pk_len - ETH_HRD_SZ, ETH_P_IP, ae->ae_hwaddr);
  }
}
...
void ip_send_out(struct pkbuf *pkb) {
  ...
  ipdbg(IPFMT " -> " IPFMT "(%d/%d bytes)", ipfmt(iphdr->ip_src), ipfmt(iphdr->ip_dst), iphlen(iphdr), _ntohs(iphdr->ip_len));
  /* ip fragment */
...
void ip_send_info(struct pkbuf *pkb, unsigned char tos, unsigned short len, unsigned char ttl, unsigned char pro, unsigned int dst) {
  struct ip *iphdr = pkb2ip(pkb);
