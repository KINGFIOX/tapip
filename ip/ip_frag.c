...
static inline int full_frag(struct fragment *frag) { return (((frag->frag_flags & FRAG_FL_IN) == FRAG_FL_IN) && (frag->frag_rsize == frag->frag_size)); }

static inline int complete_frag(struct fragment *frag) { return frag->frag_flags & FRAG_COMPLETE; }
...
struct fragment *lookup_frag(struct ip *iphdr) {
  struct fragment *frag;
  list_for_each_entry(frag, &frag_head, frag_list) if (frag->frag_id == iphdr->ip_id && frag->frag_pro == iphdr->ip_pro && frag->frag_src == iphdr->ip_src &&
                                                       frag->frag_dst == iphdr->ip_dst) return frag;
  return NULL;
}
...
struct pkbuf *ip_reass(struct pkbuf *pkb) {
  ...
  ipdbg("ID:%d RS:%d DF:%d MF:%d OFF:%d bytes size:%d bytes", iphdr->ip_id, (iphdr->ip_fragoff & IP_FRAG_RS) ? 1 : 0, (iphdr->ip_fragoff & IP_FRAG_DF) ? 1 : 0,
        (iphdr->ip_fragoff & IP_FRAG_MF) ? 1 : 0, ipoff(iphdr), iphdr->ip_len);

  frag = lookup_frag(iphdr);
  if (frag == NULL) frag = new_frag(iphdr);
  if (insert_frag(pkb, frag) < 0) return NULL;
#ifdef ICMP_EXC_FRAGTIME_TEST
  return NULL;
#endif
  if (complete_frag(frag))
...
struct pkbuf *ip_frag(struct pkbuf *pkb, struct ip *orig, int hlen, int dlen, int off, unsigned short mf_bit) {
  struct pkbuf *fragpkb;
  struct ip *fraghdr;
...
void ip_send_frag(struct netdev *dev, struct pkbuf *pkb) {
  ...
  if (dlen) {
    ...
    fragpkb = ip_frag(pkb, iphdr, hlen, dlen, off, iphdr->ip_fragoff & IP_FRAG_MF);
    ip_send_dev(dev, fragpkb);
  }
  free_pkb(pkb);
}
