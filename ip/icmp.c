...
static struct icmp_desc icmp_table[ICMP_T_MAXNUM + 1] = {[ICMP_T_ECHORLY] =
                                                             {
                                                                 .error = 0,
...
static const char *redirectstr[4] = {[ICMP_REDIRECT_NET] = "net redirect",
...
static void icmp_redirect(struct icmp_desc *icmp_desc, struct pkbuf *pkb) {
  ...
  if (icmphdr->icmp_code > 4)
    ...
    icmpdbg("from " IPFMT " %s(new nexthop " IPFMT ")", ipfmt(iphdr->ip_src), redirectstr[icmphdr->icmp_code], ipfmt(icmphdr->icmp_gw));
  free_pkb(pkb);
}
...
static void icmp_echo_reply(struct icmp_desc *icmp_desc, struct pkbuf *pkb) {
  struct ip *iphdr = pkb2ip(pkb);
  struct icmp *icmphdr = ip2icmp(iphdr);
  icmpdbg("from " IPFMT " id %d seq %d ttl %d", ipfmt(iphdr->ip_src), _ntohs(icmphdr->icmp_id), _ntohs(icmphdr->icmp_seq), iphdr->ip_ttl);
  free_pkb(pkb);
}
...
static void icmp_echo_request(struct icmp_desc *icmp_desc, struct pkbuf *pkb) {
  struct ip *iphdr = pkb2ip(pkb);
  struct icmp *icmphdr = ip2icmp(iphdr);
  icmpdbg("echo request data %d bytes icmp_id %d icmp_seq %d", (int)(iphdr->ip_len - iphlen(iphdr) - ICMP_HRD_SZ), _ntohs(icmphdr->icmp_id),
...
static void icmp_drop_reply(struct icmp_desc *icmp_desc, struct pkbuf *pkb) {
  struct ip *iphdr = pkb2ip(pkb);
  struct icmp *icmphdr = ip2icmp(iphdr);
  icmpdbg("icmp type %d code %d (dropped)", icmphdr->icmp_type, icmphdr->icmp_code);
  if (icmp_desc->info) icmpdbg("%s", icmp_desc->info);
  free_pkb(pkb);
}
...
void icmp_in(struct pkbuf *pkb) {
  ...
  if (type > ICMP_T_MAXNUM) {
    icmpdbg("unknown icmp type %d code %d", type, icmphdr->icmp_code);
    goto drop_pkb;
  }
  /* handle icmp type */
...
void icmp_send(unsigned char type, unsigned char code, unsigned int data, struct pkbuf *pkb_in) {
  struct pkbuf *pkb;
  struct ip *iphdr = pkb2ip(pkb_in);
  struct icmp *icmphdr;
  int paylen = _ntohs(iphdr->ip_len); /* icmp payload length */
  if (paylen < iphlen(iphdr) + 8) return;
  /*
   ...
   */
  if (pkb_in->pk_type != PKT_LOCALHOST) return;
  if (MULTICAST(iphdr->ip_dst) || BROADCAST(iphdr->ip_dst)) return;
  if (iphdr->ip_fragoff & _htons(IP_FRAG_OFF)) return;

  if (icmp_type_error(type) && iphdr->ip_pro == IP_P_ICMP) {
    icmphdr = ip2icmp(iphdr);
    if (icmphdr->icmp_type > ICMP_T_MAXNUM || icmp_error(icmphdr)) return;
  }
  /* build icmp packet and send */
  /* ip packet size must be smaller than 576 bytes */
  if (IP_HRD_SZ + ICMP_HRD_SZ + paylen > 576) paylen = 576 - IP_HRD_SZ - ICMP_HRD_SZ;
  ...
  icmphdr->icmp_cksum = icmp_chksum((unsigned short *)icmphdr, ICMP_HRD_SZ + paylen);
  icmpdbg("to " IPFMT "(payload %d) [type %d code %d]\n", ipfmt(iphdr->ip_src), paylen, type, code);
  ip_send_info(pkb, 0, IP_HRD_SZ + ICMP_HRD_SZ + paylen, 0, IP_P_ICMP, iphdr->ip_src);
}
