...
struct rtentry *rt_lookup(unsigned int ipaddr) {
  ...
  list_for_each_entry(rt, &rt_head, rt_list) {
    if ((rt->rt_netmask & ipaddr) == (rt->rt_netmask & rt->rt_net)) return rt;
  }
  return NULL;
}

struct rtentry *rt_alloc(unsigned int net, unsigned int netmask, unsigned int gw, int metric, unsigned int flags, struct netdev *dev) {
  struct rtentry *rt;
  rt = malloc(sizeof(*rt));
  rt->rt_net = net;
...
void rt_add(unsigned int net, unsigned int netmask, unsigned int gw, int metric, unsigned int flags, struct netdev *dev) {
...
int rt_output(struct pkbuf *pkb) {
  ...
  ipdbg("Find route entry from " IPFMT " to " IPFMT, ipfmt(iphdr->ip_src), ipfmt(iphdr->ip_dst));
  return 0;
}
...
void rt_traverse(void) {
  ...
  if (list_empty(&rt_head)) return;
  printf("Destination     Gateway         Genmask         Metric Iface\n");
  list_for_each_entry(rt, &rt_head, rt_list) {
    if (rt->rt_flags & RT_LOCALHOST) continue;
