...
void ip_forward(struct pkbuf *pkb) {
  ...
  goto drop_pkb;
#endif
  ipdbg(IPFMT " -> " IPFMT "(%d/%d bytes) forwarding", ipfmt(iphdr->ip_src), ipfmt(iphdr->ip_dst), iphlen(iphdr), _ntohs(iphdr->ip_len));
  ...
  if (indev == rt->rt_dev) {
    ...
    if (srt && srt->rt_metric == 0 && equsubnet(srt->rt_netmask, iphdr->ip_src, dst)) {
      if (srt->rt_dev != indev) {
