...
int main(void) {
  ...
  n = 10000;
  while (n > 0) {
    if (write_cbuf(cbuf, "h", 1) != 1) die("write_cbuf");
    if (read_cbuf(cbuf, buf, 1) != 1) die("read_cbuf");
    if (buf[0] != 'h') die("Not `h`");
    if (read_cbuf(cbuf, buf, 1) != 0) die("read_cbuf");
    n--;
  }
  free_cbuf(cbuf);
  ...
  /* test 16 bytes */
  cbuf = alloc_cbuf(16);
  if (write_cbuf(cbuf, "0123456789abcdefXYZ", 19) != 16) die("write_cbuf");
  if (read_cbuf(cbuf, buf, 16) != 16) die("read_cbuf");
  if (strncmp(buf, "0123456789abcdefXYZ", 16)) die("data error");
  if (cbuf->head != 0 || cbuf->tail != 0) die("cbuf point corrupts");
  /* buffer wrap */
  if (write_cbuf(cbuf, "XXXXXXXX", 8) != 8) die("write_cbuf");
  if (read_cbuf(cbuf, buf, 4) != 4) die("read_cbuf");
  if (write_cbuf(cbuf, "0123456789abcdefXYZ", 12) != 12) die("write_cbuf");
  if (read_cbuf(cbuf, buf, 16) != 16) die("read_cbuf");
  if (strncmp(buf, "XXXX0123456789abcdefXYZ", 16)) die("data error");
  free_cbuf(cbuf);
  ...
  while (i > 0) {
    buf2[i] = 'k';
    if (write_cbuf(cbuf, buf2, i * 7) != i * 7) die("write_cbuf");
    if (read_cbuf(cbuf, buf, i * 7) != i * 7) die("read_cbuf");
    if (strncmp(buf, buf2, i * 7)) die("data error");
    n--;
    if (n <= 0) {
      n = 99999 * 10;
      i--;
