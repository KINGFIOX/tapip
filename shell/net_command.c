...
void arpcache(int argc, char **argv) { arp_cache_traverse(); }

void route(int argc, char **argv) { rt_traverse(); }
...
void ifinfo(struct netdev *dev) {
  printf("%-10sHWaddr " MACFMT
         ...
         dev->net_name, macfmt(dev->net_hwaddr), ipfmt(dev->net_ipaddr), dev->net_mtu, dev->net_stats.rx_packets, dev->net_stats.rx_bytes,
         dev->net_stats.rx_errors, dev->net_stats.tx_packets, dev->net_stats.tx_bytes, dev->net_stats.tx_errors);
}
...
void stat(int argc, char **argv) {
  printf(
      "[pkbuf memory information]\n"
      " alloced pkbs: %d\n"
      " free pkbs:    %d\n",
      alloc_pkbs, free_pkbs);
  printf(
      "[sock memory information]\n"
      " alloced socks: %d\n"
      " free socks:    %d\n",
      alloc_socks, free_socks);
  printf(
      "[cbuf memory information]\n"
