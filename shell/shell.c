...
static void builtin_clear(int argc, char **argv) { printf("\033[1H\033[2J"); }

static void builtin_help(int argc, char **argv) {
  struct command *cmd;
  int i;
  for (i = 1, cmd = &cmds[0]; cmd->cmd_num; i++, cmd++) printf(" %d  %s: %s\n", i, cmd->cmd_str, cmd->cmd_help);
}
...
static int get_line(char *buf, int bufsz) {
  ...
  if (!p) {
     ...
     */
    if (errno && errno != EINTR) perrx("fgets");
...
static int parse_line(char *line, int len, char **argv) {
  ...
  p = pp = line;
  argc = 0;
  while ((p = get_arg(&pp)) != NULL) argv[argc++] = p;
  return argc;
}
...
static void parse_args(int argc, char **argv) {
  ...
  for (cmd = &cmds[0]; cmd->cmd_num; cmd++) {
    if (strcmp(cmd->cmd_str, argv[0]) == 0) goto runcmd;
  }
...
runcmd:
  if (cmd->cmd_num != CMD_NONUM && cmd->cmd_num != argc) {
    ...
    pending_work = &work;
    pthread_cond_signal(&worker_cond);
    while (pending_work) pthread_cond_wait(&master_cond, &master_mutex);
    signal_init();
  } else {
...
static void signal_init(void) {
  ...
  if ((sigaction(SIGINT, &act, NULL)) == -1) perrx("sigaction SIGINT");

  /* quit signal */
  memset(&act, 0x0, sizeof(act));
  act.sa_flags = SA_RESTART;
  act.sa_sigaction = signal_int;
  if ((sigaction(SIGQUIT, &act, NULL)) == -1) perrx("sigaction SIGOUT");
}
...
void shell_master(char *prompt_str) {
  ...
  /* shell master */
  if (prompt_str && *prompt_str) prompt = prompt_str;
  signal_init();
  while (!master_quit) {
    print_prompt();
