...
pthread_t newthread(pfunc_t thread_func) {
  pthread_t tid;
  if (pthread_create(&tid, NULL, thread_func, NULL)) perrx("pthread_create");
  return tid;
}
...
void net_stack_exit(void) {
  if (pthread_cancel(threads[0])) perror("kill child 0");
  if (pthread_cancel(threads[1])) perror("kill child 1");
  if (pthread_cancel(threads[2])) perror("kill child 2");
  /* shell work will be killed by shell master */
  if (pthread_join(threads[3], NULL)) perror("kill child 3");
  netdev_exit();
}
