...
static _inline void udp_htable_lock(void) { pthread_mutex_lock(&udp_table.mutex); }

static _inline void udp_htable_unlock(void) { pthread_mutex_unlock(&udp_table.mutex); }

static _inline int __port_used(unsigned short port, struct hlist_head *head) {
  struct hlist_node *node;
  struct sock *sk;
  hlist_for_each_sock(sk, node, head) if (sk->sk_sport == _htons(port)) return 1;
  return 0;
}

static _inline int port_used(unsigned short port) { return __port_used(port, udp_slot_head(port)); }
...
static _inline unsigned short udp_get_best_port(void) {
  ...
  if (best->used) {
    while (port < UDP_PORT_MAX) {
      if (!__port_used(port, &best->head)) break;
      port += UDP_HASH_SIZE;
    }
    if (port >= UDP_PORT_MAX) return 0;
  }
  best->used++;
  return port;
}
...
static int udp_get_port_slow(void) {
  ...
  if (best_slot == udp_table.best_slot) return 0;
  /* update best slot */
...
static unsigned short udp_get_port(void) {
  ...
  if (port == 0) return udp_get_port_slow();
...
static int udp_set_sport(struct sock *sk, unsigned short nport) {
   ...
   */
  if ((nport && port_used(_ntohs(nport))) || (!nport && !(nport = udp_get_port()))) goto unlock;
...
static int udp_init_pkb(struct sock *sk, struct pkbuf *pkb, void *buf, int size, struct sock_addr *skaddr) {
  ...
  udpdbg(IPFMT
         ":%d"
         "->" IPFMT ":%d(proto %d)",
         ipfmt(iphdr->ip_src), _ntohs(udphdr->src), ipfmt(iphdr->ip_dst), _ntohs(udphdr->dst), iphdr->ip_pro);
...
static int udp_send_buf(struct sock *sk, void *buf, int size, struct sock_addr *skaddr) {
  ...
  if (size <= 0 || size > UDP_MAX_BUFSZ) return -1;
  if (skaddr) {
  ...
  if (!sk_addr.dst_addr || !sk_addr.dst_port) return -1;
  if (!sk->sk_sport && sock_autobind(sk) < 0) return -1;
  /* udp packet send */
...
struct sock *udp_lookup_sock(unsigned short nport) {
  ...
  hlist_for_each_sock(sk, node, head) if (sk->sk_sport == nport) return get_sock(sk);
  return NULL;
}
...
struct sock *udp_alloc_sock(int protocol) {
  ...
  if (protocol && protocol != IP_P_UDP) return NULL;
