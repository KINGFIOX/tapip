...
void udp_in(struct pkbuf *pkb) {
  ...
  if (udplen > _ntohs(udphdr->length)) udplen = _ntohs(udphdr->length);
  if (udphdr->checksum && udp_chksum(iphdr->ip_src, iphdr->ip_dst, udplen, (unsigned short *)udphdr) != 0) {
  ...
  udpdbg("from " IPFMT
         ":%d"
         " to " IPFMT ":%d",
         ipfmt(iphdr->ip_src), _ntohs(udphdr->src), ipfmt(iphdr->ip_dst), _ntohs(udphdr->dst));
