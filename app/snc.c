...
#define debug(fmt, args...)                                 \
  do {                                                      \
    if (flags & F_DEBUG) dbg("(%s): " fmt, prostr, ##args); \
  } while (0)
...
static void recv_udp_packet(void) {
  ...
  while (1) {
    ...
    iphdr = pkb2ip(pkb);
    udphdr = ip2udp(iphdr);
    debug("ip: %d bytes from " IPFMT ":%d", ipdlen(iphdr), ipfmt(iphdr->ip_src), _ntohs(udphdr->src));
    /* output stdin */
...
static void recv_packet(void) {
  ...
  debug("bind " IPFMT ":%d", ipfmt(sock->sk->sk_saddr), _ntohs(sock->sk->sk_sport));
...
static int parse_args(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "b:c:du?h")) != -1) {
    switch (c) {
      case 'd':
        ...
        break;
      case 'b':
        err = parse_ip_port(optarg, &skaddr.src_addr, &skaddr.src_port);
        flags |= F_BIND;
        break;
      case 'c':
        err = parse_ip_port(optarg, &skaddr.dst_addr, &skaddr.dst_port);
        flags |= F_CONNECT;
        break;
      case 'u':
        flags &= ~F_TCP;
  ...
  if ((flags & (F_BIND | F_CONNECT)) == (F_BIND | F_CONNECT)) return -1;
  if ((flags & (F_BIND | F_CONNECT)) == 0) return -1;
  argc -= optind;
  argv += optind;
  if (argc > 0) return -1;
  if (flags & F_UDP) {
...
void snc(int argc, char **argv) {
  ...
  /* signal install */
  if (init_signal() < 0) goto out;
  /* init socket */
  if (create_socket() < 0) goto out;
  /* receive reply */
  if (flags & F_BIND)
    recv_packet();
  else
