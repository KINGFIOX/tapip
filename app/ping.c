...
static int parse_args(int argc, char **argv) {
  ...
  if (finite && count <= 0) printf("bad number of packets to transmit\n");
...
static void send_packet(void) {
  if (!buf) buf = xmalloc(size + ICMP_HRD_SZ);
  struct icmp *icmphdr = (struct icmp *)buf;
  static int first = 1;
  if (first) {
    printf("PING " IPFMT " %d(%d) bytes of data\n", ipfmt(ipaddr), size, (int)(size + ICMP_HRD_SZ + IP_HRD_SZ));
    first = 0;
  }
  ...
  icmphdr->icmp_cksum = icmp_chksum((unsigned short *)icmphdr, ICMP_HRD_SZ + size);
  seq++;
  /* socket apis */
...
static void recv_packet(void) {
  ...
  while (!finite || recv > 0) {
    ...
    if (iphdr->ip_pro == IP_P_ICMP && _ntohs(icmphdr->icmp_id) == id && icmphdr->icmp_type == ICMP_T_ECHORLY) {
      recv--;
      printf("%d bytes from " IPFMT ": icmp_seq=%d ttl=%d\n", ipdlen(iphdr), ipfmt(iphdr->ip_src), _ntohs(icmphdr->icmp_seq), iphdr->ip_ttl);
      precv++;
    }
    free_pkb(pkb);
  }
