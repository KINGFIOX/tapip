...
struct tcp {
  ...
  unsigned short reserved : 4, doff : 4,  /* data offset(head length)in 32 bits long */
                                          /* control bits */
      fin : 1, syn : 1, rst : 1, psh : 1, /* push */
      ack : 1,                            /* acknowlegment */
      urg : 1,                            /* urgent */
      ece : 1,                            /* See RFC 3168 */
      cwr : 1;                            /* See RFC 3168 */
...
static _inline int tcp_accept_queue_full(struct tcp_sock *tsk) { return (tsk->accept_backlog >= tsk->backlog); }

static _inline void tcp_accept_enqueue(struct tcp_sock *tsk) {
  /* move it from listen queue to accept queue */
  if (!list_empty(&tsk->list)) list_del(&tsk->list);
...
#define for_each_tcp_sock(tsk, node, head) hlist_for_each_entry(tsk, node, head, bhash_list)

/* used by tcp_recv_text() & tcp_reass() */
#define ADJACENT_SEGMENT_HEAD(nseq)                 \
  do {                                              \
    if ((nseq) > seg->seq) {                        \
      if (seg->dlen <= (nseq) - seg->seq) goto out; \
...
static _inline void tcp_set_state(struct tcp_sock *tsk, enum tcp_state state) {
  tcpsdbg("State from %s to %s", tcp_state_string[tsk->state], tcp_state_string[state]);
  tsk->state = state;
}
