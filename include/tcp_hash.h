...
/* reference to Linux */
static __inline unsigned int tcp_ehashfn(unsigned int src, unsigned int dst, unsigned short src_port, unsigned short dst_port) {
  unsigned int hash;
...
static _inline int tcp_ehash_conflict(struct hlist_head *head, struct sock *sk) {
  ...
  hlist_for_each_sock(tmpsk, node, head) {
    if (sk->sk_saddr == tmpsk->sk_saddr && sk->sk_daddr == tmpsk->sk_daddr && sk->sk_sport == tmpsk->sk_sport && sk->sk_dport == tmpsk->sk_dport) return 1;
  }
  return 0;
}
