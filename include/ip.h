...
#define frag_head_pkb(frag) list_first_entry(&(frag)->frag_pkb, struct pkbuf, pk_list)

#define MULTICAST(netip) ((0x000000f0 & (netip)) == 0x000000e0)
#define BROADCAST(netip) (((0xff000000 & (netip)) == 0xff000000) || ((0xff000000 & (netip)) == 0x00000000))
static inline int equsubnet(unsigned int mask, unsigned int ip1, unsigned int ip2) { return ((mask & ip1) == (mask & ip2)); }
...
extern void ip_send_info(struct pkbuf *, unsigned char, unsigned short, unsigned char, unsigned char, unsigned int);
