...
static int tcp_init_pkb(struct tcp_sock *tsk, struct pkbuf *pkb, unsigned int saddr, unsigned int daddr) {
  ...
  if (tsk && tsk->sk.sk_dst) {
    ...
    if (rt_output(pkb) < 0) return -1;
    if (tsk) tsk->sk.sk_dst = pkb->pk_rtdst;
  }
  iphdr->ip_src = saddr;
  return 0;
}
...
void tcp_send_reset(struct tcp_sock *tsk, struct tcp_segment *seg) {
  ...
  otcp->rst = 1;
  tcpdbg("send RESET from " IPFMT ":%d to " IPFMT ":%d", ipfmt(seg->iphdr->ip_dst), _ntohs(otcp->src), ipfmt(seg->iphdr->ip_src), _ntohs(otcp->dst));
...
void tcp_send_ack(struct tcp_sock *tsk, struct tcp_segment *seg) {
  ...
  tcpdbg("send ACK(%u) [WIN %d] to " IPFMT ":%d", _ntohl(otcp->ackn), _ntohs(otcp->window), ipfmt(seg->iphdr->ip_src), _ntohs(otcp->dst));
...
void tcp_send_synack(struct tcp_sock *tsk, struct tcp_segment *seg) {
  ...
  otcp->ack = 1;
  otcp->window = _htons(tsk->rcv_wnd);
  tcpdbg("send SYN(%u)/ACK(%u) [WIN %d] to " IPFMT ":%d", _ntohl(otcp->seq), _ntohs(otcp->window), _ntohl(otcp->ackn), ipfmt(seg->iphdr->ip_dst),
         _ntohs(otcp->dst));
  tcp_send_out(tsk, opkb, seg);
}
...
void tcp_send_syn(struct tcp_sock *tsk, struct tcp_segment *seg) {
  ...
  otcp->syn = 1;
  otcp->window = _htons(tsk->rcv_wnd);
  tcpdbg("send SYN(%u) [WIN %d] to " IPFMT ":%d", _ntohl(otcp->seq), _ntohs(otcp->window), ipfmt(tsk->sk.sk_daddr), _ntohs(otcp->dst));
  tcp_send_out(tsk, opkb, seg);
}
...
void tcp_send_fin(struct tcp_sock *tsk) {
  ...
  otcp->ackn = _htonl(tsk->rcv_nxt);
  otcp->ack = 1;
  tcpdbg("send FIN(%u)/ACK(%u) [WIN %d] to " IPFMT ":%d", _ntohl(otcp->seq), _ntohl(otcp->ackn), _ntohs(otcp->window), ipfmt(tsk->sk.sk_daddr),
         _ntohs(otcp->dst));
