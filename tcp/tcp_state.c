...
const char *tcp_state_string[TCP_MAX_STATE] = {
    "Unknown tcp state: 0", "CLOSED",   "LISTEN",     "SYN-RECV",   "SYN-SENT", "ESTABLISHED",
...
unsigned int alloc_new_iss(void) {
  ...
  if (++iss >= 0xffffffff) iss = 12345678;
  return iss;
}

static struct tcp_sock *tcp_listen_child_sock(struct tcp_sock *tsk, struct tcp_segment *seg) {
...
static void tcp_closed(struct tcp_sock *tsk, struct pkbuf *pkb, struct tcp_segment *seg) {
  tcpsdbg("CLOSED");
  /*
 ...
 */
static void tcp_synsent(struct pkbuf *pkb, struct tcp_segment *seg, struct tcp_sock *tsk) {
  struct tcp *tcphdr = seg->tcphdr;
  tcpsdbg("SYN-SENT");
...
static int tcp_synrecv_ack(struct tcp_sock *tsk) {
  ...
  if (tsk->parent->state != TCP_LISTEN) return -1;
  if (tcp_accept_queue_full(tsk->parent)) return -1;
...
static int seq_check(struct tcp_segment *seg, struct tcp_sock *tsk) {
  ...
  if (seg->seq < rcv_end && tsk->rcv_nxt <= seg->lastseq) return 0;
...
static _inline void tcp_update_window(struct tcp_sock *tsk, struct tcp_segment *seg) {
  if ((tsk->snd_una <= seg->ack && seg->ack <= tsk->snd_nxt) && (tsk->snd_wl1 < seg->seq || (tsk->snd_wl1 == seg->seq && tsk->snd_wl2 <= seg->ack)))
...
void tcp_process(struct pkbuf *pkb, struct tcp_segment *seg, struct sock *sk) {
  ...
  struct tcp *tcphdr = seg->tcphdr;
  tcp_dbg_state(tsk);
  if (!tsk || tsk->state == TCP_CLOSED) return tcp_closed(tsk, pkb, seg);
  if (tsk->state == TCP_LISTEN) return tcp_listen(pkb, seg, tsk);
  if (tsk->state == TCP_SYN_SENT) return tcp_synsent(pkb, seg, tsk);
  if (tsk->state >= TCP_MAX_STATE) goto drop;
  /* first check sequence number */
  tcpsdbg("1. check seq");
  if (seq_check(seg, tsk) < 0) {
    /* incoming segment is not acceptable */
    if (!tcphdr->rst) tsk->flags |= TCP_F_ACKNOW; /*reply ACK seq=snd.nxt, ack=rcv.nxt*/
    goto drop;
  }
  /* second check the RST bit */
  tcpsdbg("2. check rst");
    ...
    switch (tsk->state) {
      case TCP_SYN_RECV:
        if (tsk->parent) { /* passive open */
           ...
           * XXX: test
           */
          if (tsk->wait_connect) wake_up(tsk->wait_connect);
        }
        break;
  ...
  if (tcphdr->syn) {
     ...
     */
    if (tsk->state == TCP_SYN_RECV && tsk->parent) tcp_unhash(&tsk->sk);
    tcp_set_state(tsk, TCP_CLOSED);
    free_sock(&tsk->sk);
  }
  ...
  switch (tsk->state) {
    ...
    case TCP_FIN_WAIT1:
    case TCP_CLOSING:
      tcpsdbg("SND.UNA %u < SEG.ACK %u <= SND.NXT %u", tsk->snd_una, seg->ack, tsk->snd_nxt);
      if (tsk->snd_una < seg->ack && seg->ack <= tsk->snd_nxt) {
        tsk->snd_una = seg->ack;
        /*
  ...
  switch (tsk->state) {
    ...
    case TCP_FIN_WAIT1:
    case TCP_FIN_WAIT2:
      if (tcphdr->psh || seg->dlen > 0) tcp_recv_text(tsk, seg, pkb);
      break;
      /*
...
drop:
  ...
  if (tsk->flags & (TCP_F_ACKNOW | TCP_F_ACKDELAY)) tcp_send_ack(tsk, seg);
  free_pkb(pkb);
}
