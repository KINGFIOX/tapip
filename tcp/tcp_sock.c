...
static struct sock *tcp_lookup_sock_establish(unsigned int src, unsigned int dst, unsigned short src_port, unsigned short dst_port) {
  struct hlist_head *head;
  struct hlist_node *node;
  struct sock *sk;
  head = tcp_ehash_head(tcp_ehashfn(src, dst, src_port, dst_port));
  hlist_for_each_sock(sk, node, head) {
    if (sk->sk_saddr == dst && sk->sk_daddr == src && sk->sk_sport == dst_port && sk->sk_dport == src_port) return get_sock(sk);
  }
  return NULL;
}
...
static struct sock *tcp_lookup_sock_listen(unsigned int addr, unsigned int nport) {
  ...
  hlist_for_each_sock(sk, node, head) {
    if ((!sk->sk_saddr || sk->sk_saddr == addr) && sk->sk_sport == nport) return get_sock(sk);
  }
  return NULL;
}

/* port is net order! */
struct sock *tcp_lookup_sock(unsigned int src, unsigned int dst, unsigned int src_port, unsigned int dst_port) {
  struct sock *sk;
  sk = tcp_lookup_sock_establish(src, dst, src_port, dst_port);
  if (!sk) sk = tcp_lookup_sock_listen(dst, dst_port);
  return sk;
}
...
static _inline int __tcp_port_used(unsigned short nport, struct hlist_head *head) {
  struct hlist_node *node;
  struct tcp_sock *tsk;
  for_each_tcp_sock(tsk, node, head) if (tsk->sk.sk_sport == nport) return 1;
  return 0;
}

static _inline int tcp_port_used(unsigned short nport) { return __tcp_port_used(nport, tcp_bhash_head(_ntohs(nport) & TCP_BHASH_MASK)); }

static unsigned short tcp_get_port(void) {
  static unsigned short defport = TCP_BPORT_MIN;
  unsigned short nport = 0;
  /* no free bind port resource */
  if (tcp_table.bfree <= 0) return 0;
  /* assert that we can break the loop */
  while (tcp_port_used(_htons(defport))) {
    if (++defport > TCP_BPORT_MAX) defport = TCP_BPORT_MIN;
  }
...
static int tcp_set_sport(struct sock *sk, unsigned short nport) {
  ...
  if ((nport && tcp_port_used(nport)) || (!nport && !(nport = tcp_get_port()))) goto out;
  tcp_table.bfree--;
  sk->sk_sport = nport;
...
static void tcp_unset_sport(struct sock *sk) { tcp_table.bfree++; }
...
int tcp_hash(struct sock *sk) {
  ...
  if (tsk->state == TCP_LISTEN) {
    ...
    hash = tcp_ehashfn(sk->sk_saddr, sk->sk_daddr, sk->sk_sport, sk->sk_dport);
    head = tcp_ehash_head(hash);
    if (tcp_ehash_conflict(head, sk)) return -1;
    sk->hash = hash;
  }
...
static _inline void tcp_pre_wait_connect(struct tcp_sock *tsk) { tsk->wait_connect = &tsk->sk.sock->sleep; }
...
static int tcp_listen(struct sock *sk, int backlog) {
  ...
  if (oldstate != TCP_CLOSED && oldstate != TCP_LISTEN) return -1;
  tsk->backlog = backlog;
  tsk->state = TCP_LISTEN;
  /* add tcpsk into listen hash table */
  if (oldstate != TCP_LISTEN && sk->ops->hash) sk->ops->hash(sk);
  return 0;
}
...
static struct sock *tcp_accept(struct sock *sk) {
  ...
  while (list_empty(&tsk->accept_queue)) {
    if (tcp_wait_accept(tsk) < 0) goto out;
  }
...
static int tcp_close(struct sock *sk) {
  ...
  switch (tsk->state) {
    ...
    case TCP_LISTEN:
      ...
      if (sk->ops->unhash) sk->ops->unhash(sk);
      tcp_unbhash(tsk);
      tsk->state = TCP_CLOSED;
      break;
    case TCP_SYN_RECV:
...
static int tcp_send_buf(struct sock *sk, void *buf, int len, struct sock_addr *saddr) {
  struct tcp_sock *tsk = tcpsk(sk);
  int ret = -1;
  switch (tsk->state) {
    case TCP_CLOSED:
...
static int tcp_recv_buf(struct sock *sk, char *buf, int len) {
  ...
  switch (tsk->state) {
    ...
    case TCP_CLOSED:
      goto out;
    case TCP_CLOSE_WAIT:
      if (!tsk->rcv_buf || !CBUFUSED(tsk->rcv_buf)) goto out;
    case TCP_ESTABLISHED:
    case TCP_FIN_WAIT1:
    case TCP_FIN_WAIT2:
      break;
  ...
  while (rlen < len) {
    ...
    rlen += curlen;
    /* wait buffer filled */
    while (!((tsk->flags & TCP_F_PUSH) || (tsk->rcv_buf && CBUFUSED(tsk->rcv_buf)) || (rlen >= len))) {
    ...
    if ((tsk->flags & TCP_F_PUSH) && !(tsk->rcv_buf && CBUFUSED(tsk->rcv_buf))) {
...
static void tcp_recv_notify(struct sock *sk) {
  if (sk->recv_wait) wake_up(sk->recv_wait);
}
...
struct sock *tcp_alloc_sock(int protocol) {
  struct tcp_sock *tsk;
  if (protocol && protocol != IP_P_TCP) return NULL;
...
void tcp_init(void) {
  int i;
  for (i = 0; i < TCP_EHASH_SIZE; i++) hlist_head_init(&tcp_table.etable[i]);
  for (i = 0; i < TCP_LHASH_SIZE; i++) hlist_head_init(&tcp_table.ltable[i]);
