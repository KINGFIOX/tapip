...
void tcp_recv_text(struct tcp_sock *tsk, struct tcp_segment *seg, struct pkbuf *pkb) {
  ...
  if (tsk->rcv_nxt == seg->seq && list_empty(&tsk->rcv_reass)) {
    ...
    if (rlen > 0 && seg->tcphdr->psh) tsk->flags |= TCP_F_PUSH;
...
out:
  if (tsk->flags & TCP_F_PUSH) tsk->sk.ops->recv_notify(&tsk->sk);
}

static void tcp_init_text(struct tcp_sock *tsk, struct pkbuf *pkb, void *buf, int size) {
  ...
  tsk->snd_nxt += size;
  tsk->snd_wnd -= size;
  tcpsdbg("send TEXT(%u:%d) [WIN %d] to " IPFMT ":%d", _ntohl(tcphdr->seq), size, _ntohs(tcphdr->window), ipfmt(tsk->sk.sk_daddr), _ntohs(tcphdr->dst));
}
