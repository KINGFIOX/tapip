...
static void tcp_segment_init(struct tcp_segment *seg, struct ip *iphdr, struct tcp *tcphdr) {
  ...
  tcpdbg("from " IPFMT
         ...
         ":%d"
         "\tseq:%u(%d:%d) ack:%u %s",
         ipfmt(iphdr->ip_src), _ntohs(tcphdr->src), ipfmt(iphdr->ip_dst), _ntohs(tcphdr->dst), _ntohl(tcphdr->seq), seg->dlen, seg->len, _ntohl(tcphdr->ackn),
         tcp_control_string(tcphdr));
}
...
static void tcp_recv(struct pkbuf *pkb, struct ip *iphdr, struct tcp *tcphdr) {
  ...
  sk = tcp_lookup_sock(iphdr->ip_src, iphdr->ip_dst, tcphdr->src, tcphdr->dst);
...
void tcp_in(struct pkbuf *pkb) {
  ...
  if (tcp_chksum(iphdr->ip_src, iphdr->ip_dst, tcplen, (unsigned short *)tcphdr) != 0) {
