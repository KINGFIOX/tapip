...
static _inline int __raw_hash_func(int protocol) { return (protocol & IP_P_RAW); }

static _inline int raw_hash_func(struct sock *sk) { return __raw_hash_func(sk->protocol); }

static void raw_unhash(struct sock *sk) { sock_del_hash(sk); }
...
static void raw_init_pkb(struct sock *sk, struct pkbuf *pkb, struct sock_addr *skaddr) {
...
static int raw_send_buf(struct sock *sk, void *buf, int size, struct sock_addr *skaddr) {
  struct pkbuf *pkb;
  if (size < 0 || size > RAW_MAX_BUFSZ) return -1;
...
struct sock *raw_alloc_sock(int protocol) {
  ...
  if (protocol == IP_P_IP) return NULL;
...
void raw_init(void) {
  int i;
  for (i = 0; i < IP_P_MAX; i++) hlist_head_init(&raw_hash_table[i]);
  raw_id = 0;
  dbg("raw ip init");
}

static _inline struct sock *__raw_lookup_sock(struct hlist_head *head, unsigned int src, unsigned int dst, int proto) {
  struct sock *sk;
  struct hlist_node *node;
  hlist_for_each_sock(sk, node, head) {
    if ((sk->protocol == proto) && (!sk->sk_saddr || sk->sk_saddr == src) && (!sk->sk_daddr || sk->sk_daddr == dst)) return sk;
  }
  return NULL;
}

struct sock *raw_lookup_sock_next(struct sock *sk, unsigned int src, unsigned int dst, int proto) {
  struct sock *bak;
  bak = __raw_lookup_sock((struct hlist_head *)&sk->hash_list, src, dst, proto);
  return bak;
}
...
struct sock *raw_lookup_sock(unsigned int src, unsigned int dst, int proto) {
  ...
  struct sock *sk;
  if (hlist_empty(hash_head)) return NULL;
