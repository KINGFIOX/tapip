...
static struct inet_type inet_type_table[SOCK_MAX] = {[0] = {},
                                                     [SOCK_STREAM] =
                                                         {
                                                             .type = SOCK_STREAM,
                                                             .protocol = IP_P_TCP,
...
static int inet_socket(struct socket *sock, int protocol) {
  ...
  if (!protocol) protocol = inet->protocol;
  sock->sk = get_sock(sk);
  ...
  sk->sock = sock;
  /* only used by raw ip */
  if (sk->hash && sk->ops->hash) sk->ops->hash(sk);
  return 0;
}
...
static int inet_listen(struct socket *sock, int backlog) {
  ...
  if (sk) err = sk->ops->listen(sk, backlog);
  return err;
}
...
static int inet_bind(struct socket *sock, struct sock_addr *skaddr) {
  ...
  if (sk->ops->bind) return sk->ops->bind(sock->sk, skaddr);
  /* duplicate bind is error */
  if (sk->sk_sport) goto err_out;
  /* only bind local ip address */
  if (!local_address(skaddr->src_addr)) goto err_out;
  /* bind address */
...
static int inet_connect(struct socket *sock, struct sock_addr *skaddr) {
  ...
  int err = -1;
  /* sanity check */
  if (!skaddr->dst_port || !skaddr->dst_addr) goto out;
  /* Not allow double connection */
  if (sk->sk_dport) goto out;
  /* if not bind, we try auto bind */
  if (!sk->sk_sport && sock_autobind(sk) < 0) goto out;
  /* ROUTE */
  {
  ...
  if (sk->ops->connect) err = sk->ops->connect(sk, skaddr);
...
static int inet_write(struct socket *sock, void *buf, int len) {
  struct sock *sk = sock->sk;
  int ret = -1;
  if (sk) ret = sk->ops->send_buf(sock->sk, buf, len, NULL);
  return ret;
}

static int inet_send(struct socket *sock, void *buf, int size, struct sock_addr *skaddr) {
  struct sock *sk = sock->sk;
  if (sk) return sk->ops->send_buf(sock->sk, buf, size, skaddr);
  return -1;
}
