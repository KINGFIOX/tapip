...
static void free_socket(struct socket *sock) {
  if (--sock->refcnt <= 0) __free_socket(sock);
}
...
int _listen(struct socket *sock, int backlog) {
  ...
  get_socket(sock);
  if (sock->ops) err = sock->ops->listen(sock, backlog);
  free_socket(sock);
out:
  return err;
}
...
int _bind(struct socket *sock, struct sock_addr *skaddr) {
  ...
  if (!sock || !skaddr) goto out;
  get_socket(sock);
  if (sock->ops) err = sock->ops->bind(sock, skaddr);
  free_socket(sock);
out:
  return err;
}
...
struct socket *_accept(struct socket *sock, struct sock_addr *skaddr) {
  ...
  if (!newsock) goto out_free;
  newsock->ops = sock->ops;
  /* real accepting process */
  if (sock->ops) err = sock->ops->accept(sock, newsock, skaddr);
  if (err < 0) {
    free(newsock);
    newsock = NULL;
  }
...
int _send(struct socket *sock, void *buf, int size, struct sock_addr *skaddr) {
  int err = -1;
  if (!sock || !buf || size <= 0 || !skaddr) goto out;
  get_socket(sock);
  if (sock->ops) err = sock->ops->send(sock, buf, size, skaddr);
  free_socket(sock);
out:
  return err;
}
...
struct pkbuf *_recv(struct socket *sock) {
  ...
  get_socket(sock);
  if (sock->ops) pkb = sock->ops->recv(sock);
  free_socket(sock);
out:
  return pkb;
}
...
int _write(struct socket *sock, void *buf, int len) {
  int ret = -1;
  if (!sock || !buf || len <= 0) goto out;
  /* get reference for _close() safe */
  get_socket(sock);
  if (sock->ops) ret = sock->ops->write(sock, buf, len);
  free_socket(sock);
out:
  return ret;
}
...
int _read(struct socket *sock, void *buf, int len) {
  ...
  /* get reference for _close() safe */
  get_socket(sock);
  if (sock->ops) ret = sock->ops->read(sock, buf, len);
  free_socket(sock);
out:
  return ret;
}

void socket_init(void) { inet_init(); }
