...
void pkb_trim(struct pkbuf *pkb, int len) {
  pkb->pk_len = len;
  if (realloc(pkb, sizeof(*pkb) + len) == NULL) perrx("realloc");
}
...
struct pkbuf *alloc_netdev_pkb(struct netdev *nd) { return alloc_pkb(nd->net_mtu + ETH_HRD_SZ); }
...
void get_pkb(struct pkbuf *pkb) { pkb->pk_refcnt++; }
