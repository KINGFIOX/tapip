...
struct netdev *netdev_alloc(char *devstr, struct netdev_ops *netops) {
  ...
  dev->net_ops = netops;
  if (netops && netops->init) netops->init(dev);
  return dev;
}

void netdev_free(struct netdev *dev) {
  if (dev->net_ops && dev->net_ops->exit) dev->net_ops->exit(dev);
  list_del(&dev->net_list);
  free(dev);
}

void netdev_interrupt(void) { veth_poll(); }
...
#ifdef DEBUG_PKB
void _netdev_tx(struct netdev *dev, struct pkbuf *pkb, int len, unsigned short proto, unsigned char *dst)
#else
void netdev_tx(struct netdev *dev, struct pkbuf *pkb, int len, unsigned short proto, unsigned char *dst)
#endif
{
  ...
  l2dbg(MACFMT " -> " MACFMT "(%s)", macfmt(ehdr->eth_src), macfmt(ehdr->eth_dst), ethpro(proto));
...
int local_address(unsigned int addr) {
  ...
  if (!addr) return 1;
  /* loop back address */
  if (LOCALNET(loop) == (loop->net_mask & addr)) return 1;
  /* nic bind address */
  list_for_each_entry(dev, &net_devices, net_list) {
    if (dev->net_ipaddr == addr) return 1;
  }
  return 0;
}
