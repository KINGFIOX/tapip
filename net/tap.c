...
void getname_tap(int tapfd, unsigned char *name) {
  struct ifreq ifr = {};
  if (ioctl(tapfd, TUNGETIFF, (void *)&ifr) < 0) perrx("ioctl SIOCGIFHWADDR");
...
void gethwaddr_tap(int tapfd, unsigned char *ha) {
  ...
  /* get net order hardware address */
  if (ioctl(tapfd, SIOCGIFHWADDR, (void *)&ifr) < 0) perrx("ioctl SIOCGIFHWADDR");
  hwacpy(ha, ifr.ifr_hwaddr.sa_data);
  dbg("mac addr: %02x:%02x:%02x:%02x:%02x:%02x", ha[0], ha[1], ha[2], ha[3], ha[4], ha[5]);
}
...
void delete_tap(int tapfd) {
  if (ioctl(tapfd, TUNSETPERSIST, 0) < 0) return;
  close(tapfd);
}

void set_tap(void) {
  skfd = socket(PF_INET, SOCK_DGRAM, IPPROTO_IP);
  if (skfd < 0) perrx("socket PF_INET");
}

void unset_tap(void) { close(skfd); }
