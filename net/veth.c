...
static int tap_dev_init(void) {
  ...
  if (tap->fd < 0) goto free_tap;
  if (setpersist_tap(tap->fd) < 0) goto close_tap;
  /* set tap information */
  set_tap();
...
static void veth_dev_exit(struct netdev *dev) {
  if (dev != veth) perrx("Net Device Error");
  delete_tap(tap->fd);
}

static int veth_dev_init(struct netdev *dev) {
  /* init tap: out network nic */
  if (tap_dev_init() < 0) perrx("Cannot init tap device");
...
void veth_poll(void) {
  ...
  while (1) {
    ...
    ret = poll(&pfd, 1, -1);
    if (ret <= 0) perrx("poll /dev/net/tun");
    /* get a packet and handle it */
    veth_rx();
  }
}

void veth_init(void) { veth = netdev_alloc("veth", &veth_ops); }
